...
def submit():
    def convert_numerical_strings_to_int(d):
        for key, value in d.items():
            ...
            ):  # Check if value is a digit string
    ...
    f.close()
    GTOo1_prompt = GTOo1_prompt.format(
        GTO_data=jsonify(all_info), sample_response=sample_response
    )
    response = client.chat.completions.create(
        model="gpt-4o-mini", messages=[{"role": "user", "content": GTOo1_prompt}]
    )
...
def upload_file():
    ...
    if file and allowed_file(file.filename):
            ...
            "messages": [
                {
                    "role": "user",
                    "content": [
                        {"type": "text", "text": GTOextract_prompt},
                        {
                            "type": "image_url",
                            "image_url": {
                                "url": f"data:image/{image_type};base64,{base64_image}"
        ...
        f.close()
        GTOo1_prompt = GTOo1_prompt.format(
            GTO_data=GTO_data, sample_response=sample_response
        )
        response = client.chat.completions.create(
            model="gpt-4o-mini", messages=[{"role": "user", "content": GTOo1_prompt}]
        )
